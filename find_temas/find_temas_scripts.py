...
def get_base_as_df(base_file_path, coluna_texto = 'Texto'):
...
def add_temas_to_df(df, temas, coluna_texto = 'Texto'):
    df_temas = df.copy()
    for tema in temas:
         df_temas[tema['nome']] = df_temas[coluna_texto].apply(lambda x: is_about_tema(tema['regex'], x))
    return df_temas

def find_temas(base_file_path, temas_file_path, coluna_texto):
    base = get_base_as_df(base_file_path, coluna_texto)
    temas = get_temas(temas_file_path)

    base_with_temas = add_temas_to_df(base, temas, coluna_texto)
    base_with_temas.to_csv('output.csv', index=False)